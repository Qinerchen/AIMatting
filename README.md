...
- 旧的 `aimatting/ui/good_window.py` 自绘无边框窗口实现保留作参考，已不再使用。
...
- **GPU（RTX 4090 级别）**：单张 1080P 约 0.5-1.5 秒，4K 约 2-4 秒，
  10 张 1080P 约 5-15 秒，可满足需求文档中的性能指标。
- **CPU**：单张 1080P 约 5-20 秒（取决于机型与推理分辨率档位）。
  建议在「输出设置」中降低推理分辨率（512/1024）换取速度。
- 内存：HR-matting 推理约需 4-6GB 显存/内存，lite 模型约 1-2GB。
...
- **抠图结果边缘粗糙**：降低推理分辨率会损失细节；质量优先时用 HR-matting 模型
  并选择 1024/2048 档位。
- **JPG 为什么不能透明**：JPG/WEBP 格式不支持透明通道，半透明背景会平铺到白色底；
  透明素材请导出 PNG。
- **大图卡顿**：4K 以上图片建议使用 lite 模型，或先降低推理分辨率。
